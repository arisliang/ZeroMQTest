...
namespace ZeroMQTest.Common.Patterns
{
    ...
    public static class LBBroker
    {
        ...
        /// </summary>
        public static void LBBroker_Client(int i, string clientConnectAddress = "inproc://frontend")
        {
            ZError error = null;
            ...
            using (var context = ZContext.Create())
            {
                using (var client = ZSocket.Create(context, ZSocketType.REQ))
                {
                    ...
                    if (error == null)
                    {
                        LogService.Debug("{0}: client connecting to {1}.", Thread.CurrentThread.Name, clientConnectAddress);
                    }
                    else
                    {
                        LogService.Debug("{0}: client connecting to {1} with error {2}.", Thread.CurrentThread.Name, clientConnectAddress, error.ToString());
                    }
        ...
        /// </summary>
        public static void LBBroker_Worker(int i, string workerConnectAddress = "inproc://backend")
        {
            ZError error = null;
            ...
            using (var context = ZContext.Create())
            {
                ...
                using (var worker = ZSocket.Create(context, ZSocketType.REQ))
                {
                    ...
                    if (error == null)
                    {
                        LogService.Debug("{0}: worker connecting to {1}.", Thread.CurrentThread.Name, workerConnectAddress);
                    }
                    else
                    {
                        LogService.Debug("{0}: worker connecting to {1} with error {2}.", Thread.CurrentThread.Name, workerConnectAddress, error.ToString());
                    }
        ...
        /// </summary>
        public static void LBBroker_Broker(int numOfClients, string clientBindAddress = "inproc://frontend", string workerBindAddress = "inproc://backend")
        {
            ZError error = null;
            ...
            using (var context = ZContext.Create())
            {
                using (var frontend = ZSocket.Create(context, ZSocketType.ROUTER))
                {
                    ...
                    if (error == null)
                    {
                        LogService.Debug("{0}: frontend binding on {1}", Thread.CurrentThread.Name, clientBindAddress);
                    }
                    else
                    {
                        LogService.Debug("{0}: frontend binding on {1} failed with error {2}.", Thread.CurrentThread.Name, clientBindAddress, error.ToString());
                    }
                    ...
                    using (var backend = ZSocket.Create(context, ZSocketType.ROUTER))
                    {
                        ...
                        if (error == null)
                        {
                            LogService.Debug("{0}: backend binding on {1}", Thread.CurrentThread.Name, workerBindAddress);
                        }
                        else
                        {
